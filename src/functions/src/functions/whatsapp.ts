    ...
    try {
        ...
        if (!activeFlowId && message.type === 'text' && message.text?.body) {
            ...
            for (const doc of flowsSnapshot.docs) {
                ...
                const triggerNode = flow.nodes.find(n => n.data.type === 'keywordTrigger'); // Find any trigger
                ...
                if (isMatch) {
                    ...
                    // Execute the flow starting from the matched trigger node
                    const nextStep = await processFlowStep(companyId, consumerRef, activeFlow, triggerNode.id, message);
                    
                    const isSingleStepFlow = !nextStep || nextStep.data.type === 'endFlow';
